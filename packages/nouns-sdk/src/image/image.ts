...
export class Image {
  ...
  public colorMatch(colors: Map<string, number>): boolean {
    this._bounds = this.calcBounds();

    const indexes: number[] = [];

    let result = false;
    // console.log("colorMatch", this);

    for (let y = this.bounds.top; y <= this.bounds.bottom; y++) {
      // console.log("Test", this.bounds);
      for (let x = this.bounds.left; x < this.bounds.right; x++) {
        ...
        // If color doesn't exist, reject this image
        if (!colors.has(hexColor)) {
           console.log("rejected", hexColor);
          result = false;
          return result;
        }
        else {
